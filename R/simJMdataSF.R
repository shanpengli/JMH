...
simJMdataSF <- function(seed = 100, N = 200, increment = 0.7, beta = c(5, 1.5, 2, 1, 2),
  ...
  survtimeraw <- as.data.frame(cbind(survtimeraw, survtime, status))
  table <- as.data.frame(table(survtimeraw$status)/N*100)
  writeLines(paste0("The censoring rate is: ", table[1, 2], "%"))
  writeLines(paste0("The risk 1 rate is: ", table[2, 2], "%"))
  ID <- c(1:N)
  cdata <- cbind(ID, survtimeraw$survtime, survtimeraw$status, Z)
  colnames(cdata) <- c("ID", "survtime", "status", "var1", "var2", "var3")
