GetVarSurvfit <- function(cdata, ydata.mean, ydata.variance, random) {
  ...
  mdata <- as.data.frame(table(ydata.mean[, ID]))
  colnames(mdata)[1] <- ID
  mdata[, ID] <- as.character(mdata[, ID])
  cdata[, ID] <- as.character(cdata[, ID])
  ydata.mean[, ID] <- as.character(ydata.mean[, ID])
  ...
  a <- list(Z = Z, X1 = X1, W = W, Y = Y, X2 = X2, survtime = survtime, cmprsk = cmprsk, 
