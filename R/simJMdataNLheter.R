simJMdataNLheter <- function(N = 200, lambda1 = 0.05, lambda2 = 0.1,
                             beta = c(5, 1.5, 2, 1),
                             tau = c(2, 1, 1, 0.05),
                             gamma1 = c(1, 0.5, 0.5),
  ...
  for (i in 1:N) {
    ...
    suby[, 1] <- i
    sd <- sqrt(exp(tau[1] + tau[2]*Z[i, 1] + bwi[i, 4]))
    suby[1, 2] <- beta[1] + beta[2]*Z[i, 1] + bwi[i, 1] + rnorm(1, mean = 0, sd = sd)
    ...
    if (ni==0) {
      ...
      for (j in 1:ni) {
        sd <- sqrt(exp(tau[1] + tau[2]*Z[i, 1] + tau[3]*timebs[j, 1] + tau[4]*timebs[j, 2] + bwi[i, 4]))
        suby[j+1, 2] <- beta[1] + beta[2]*Z[i, 1] + beta[3]*timebs[j, 1] + beta[4]*timebs[j, 2] + 
