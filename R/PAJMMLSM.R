...
PAJMMLSM <- function(seed = 100, object, landmark.time = NULL, horizon.time = NULL, 
  ...
  for (t in 1:n.cv) {
    ...
    fit <- try(JMMLSM(cdata = train.cdata, ydata = train.ydata, 
                      ...
                      maxiter = maxiter, opt = object$opt, 
    ...
    if ('try-error' %in% class(fit)) {
      ...
      survfit <- try(survPAJMMLSM(fit, ynewdata = val.ydata, cnewdata = val.cdata,
