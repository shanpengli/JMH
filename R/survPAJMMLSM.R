...
##' @export
##' 
survPAJMMLSM <- function(object, ynewdata = NULL, cnewdata = NULL, 
  ...
  if (object$CompetingRisk) {
    ...
    for (j in 1:N.ID) {
      ...
      CIF <- getECIFall(beta, tau, gamma1, gamma2, alpha1, alpha2, nu1,
                        nu2, Sig, Z, X, W, Y, as.vector(X2), H01, H02,
                        xsmatrix, wsmatrix, CH01, CH02, s, timecif1, timecif2, Posmean, PosCov)
