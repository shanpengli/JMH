GetinitFakeSi <- function(cdata, ydata, long.formula, surv.formula, variance.formula,
                          model, ID, RE)
{
  ...
  if (sum(unique(cmprsk)) <= 2) {
    if (prod(c(0, 1) %in% unique(cmprsk))) {
      beta = c(0.5, 0.1, 0.2, -0.3, 0.5)
      tau = c(-0.05, -0.01, -0.02, 0.03, 0.01)
      gamma1 = c(-0.5, 0.2, -0.1)
      vee1 = -0.7
      
      if (model == "intercept") {
        alpha1 = as.vector(0.5)
        Sig <- matrix(c(0.5, 0.05, 0.05, 0.5), nrow = 2, ncol = 2)
      } else {
        alpha1 = c(1, 0.7)
