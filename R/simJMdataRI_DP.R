...
simJMdataRI_DP <- function(seed = 100, N = 2, increment = 0.7, beta = c(5, 1.5, 2, 1, 2),
  ...
  for (i in 1:N) {
    ...
    lam1[i] <- temp/exp(alpha1*bwi[i, 1] + vee1*bwi[i, 2])
    risk1[i] <- rexp(1, temp)
    temp=lambda2*exp(Z[i, ] %*% gamma2 + alpha2*bwi[i, 1] + vee2*bwi[i, 2])
    lam2[i] <- temp/exp(alpha2*bwi[i, 1] + vee2*bwi[i, 2])
    risk2[i] <- rexp(1, temp)
  }
  ...
  lam1 <- as.data.frame(cbind(ID, lam1))
  lam2 <- as.data.frame(cbind(ID, lam2))
  colnames(lam1) <- c("ID", "lam1")
  colnames(lam2) <- c("ID", "lam1")
  a <- list(cdata = cdata, ydata = ydata, lam1 = lam1, lam2 = lam2)
  return(a)
}
