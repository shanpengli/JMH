simJMdatahomo <- function(N = 200, lambda1 = 0.05, lambda2 = 0.1,
                          beta = c(5, 1.5, 2, 1),
                          sigma2 = exp(0.5),
                          gamma1 = c(1, 0.5, 0.5),
  ...
  bi <- MASS::mvrnorm(n = N, rep(0, 3), sigb, tol = 1e-6, empirical = FALSE)
    ...
    if (ni==0) {
      ...
      for (j in 1:ni) {
        suby[j+1, 2] <- beta[1] + beta[2]*Z[i, 1] + beta[3]*timebs[j, 1] + beta[4]*timebs[j, 2] + 
          bi[i, 1] + bi[i, 2]*timebs[j, 1] + bi[i, 3]*timebs[j, 2] + rnorm(1, mean = 0, sd = sqrt(sigma2))
