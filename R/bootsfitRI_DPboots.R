...
bootsfitRI_DPboots <- function(i, seed, N, increment, beta, tau, gamma1, gamma2,
  ...
  if ('try-error' %in% class(fit)) {
    ...
    survfit <- try(survfitJMMLSMboot(fit, seed = seed + i, ynewdata = ynewdata, cnewdata = cnewdata, 
                                  u = u, B = M, method = "GH", quadpoint = quadpoint), silent = TRUE)
