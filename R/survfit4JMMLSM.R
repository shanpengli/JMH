...
##' @export
##' 
survfit4JMMLSM <- function(object, seed = 100, ynewdata = NULL, cnewdata = NULL, 
    ...
    if (!object$CompetingRisk) {
      ...
      for (i in 1:M) {
        ...
        for (j in 1:N.ID) {
          ...
          for (jj in 1:lengthu) {
            ## calculate the CIF
            CIF <- getECIF(betal, taul, gammal1, gammal2, alphal1, alphal2, nul1,
                           nul2, Sigl, Z, X, W, Y, as.vector(X2), H01l, H02l,
