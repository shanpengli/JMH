simfitRIhomo.FastJM <- function(sim = 100, N = 200, lambda1 = 0.05, lambda2 = 0.1,
                         beta = c(5, 1.5, 2, 1, 2),
                         sigma2 = exp(0.5),
                         gamma1 = c(1, 0.5, 0.5),
  ...
  ParaMatrixRaw <- parallel::mclapply(1:sim, bootsfitRIhomo.FastJM,
  ...
  paramatrix <- as.data.frame(matrix(0, nrow = sim, ncol = 17))
  paramatrixSE <- as.data.frame(matrix(0, nrow = sim, ncol = 15))
  ...
  colnames(paramatrix)[count] <- "sigma"
  count <- count + 1
  for (i in 1:3) {
  ...
  colnames(paramatrix)[count] <- paste0("Sig_", 1, 1)
  count <- count + 1
  ...
  name <- colnames(paramatrix)[-(15:16)]
